package com.jookershop.linefriend.career;
...
public class CareerAdapter extends BaseAdapter {
	    ...
	    // Constructor
	    public CareerAdapter(Context c){
	        mContext = c;
	    }
	 
	    @Override
	    public int getCount() {
	        return Constants.CAREER_THUMBNAIL_IDS.length;
	    }
	 
	    @Override
	    public Object getItem(int position) {
	        return Constants.CAREER_THUMBNAIL_IDS[position];
	    }
...
//          	iv.setImageResource( Constants.CAREER_THUMBNAIL_IDS[position]);
	    	MainActivity.imageLoader.displayImage("drawable://" + Constants.CAREER_THUMBNAIL_IDS[position], 
	    			iv, MainActivity.options, new SimpleImageLoadingListener(), null);
          	
          	TextView title = (TextView) row.findViewById(R.id.title);
          	title.setText(Constants.CAREER_TITLES[position]);
          	...
          	final String detailTitle = mContext.getString(R.string.title_section3) + "/" + Constants.CAREER_TITLES[position];
          	row.setOnClickListener(new View.OnClickListener() {
				...
				public void onClick(View v) {
					...
					.replace(R.id.container,
							SubCategoryFragment.newInstance(Constants.TYPE_CAREER, Constants.CAREER_IDS[position]))
							.addToBackStack("career").commit();
				}
			});           	
