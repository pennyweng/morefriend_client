...
//		tabs.setTextColor(Color.parseColor("#828181"));
	...
	public class MyPagerAdapter extends FragmentPagerAdapter {
		...
		private final String[] TITLES = { "今日新進", "今日最搶手", "今日最活躍" };
		...
		public Fragment getItem(int position) {
			if(position == 0) {
				return TodayNewFragment.newInstance(position + 1);
		    } else if(position == 1){
		    	return HotNewFragment.newInstance(position + 1);
			}  else {
				return ActiveNewFragment.newInstance(position + 1);
