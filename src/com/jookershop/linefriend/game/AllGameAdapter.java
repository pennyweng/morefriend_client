...
public class AllGameAdapter extends ArrayAdapter<GameItem> {
	private Context mContext;
	private DisplayImageOptions options;
	private SharedPreferences sp;
	private ActiveGameFragment fdf;

	public AllGameAdapter(Context context, ArrayList<GameItem> interestItems,
			DisplayImageOptions options, ActiveGameFragment fdf) {
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		...
		if(postItem.getType().equals(GameItem.TYPE_GUESS)) {
			holder.title.setText("遊戲名稱：" + postItem.getName());
			holder.maintitle.setText(nn);
		} else if(postItem.getType().equals(GameItem.TYPE_AB)) {
	...
	private void checkTime(final GameItem gi, final String currentNumber) {
		String uid = URLEncoder.encode(AccountUtil.getUid(mContext));
						...
						((Activity) mContext).runOnUiThread(new Runnable() {
							public void run() {
								if (!result.equals("0") && !Constants.IS_SUPER) {
									...
									if(gameType.equals(GameItem.TYPE_GUESS))
									...
									else if (gameType.equals(GameItem.TYPE_AB))
										((FragmentActivity) mContext)
										.getSupportFragmentManager()
										.beginTransaction()
										.replace(
												...
												ABGameFragment
														.newInstance(gi))
										.addToBackStack("allgame").commit();								
									else if (gameType.equals(GameItem.TYPE_LUCKYN))
										((FragmentActivity) mContext)
										.getSupportFragmentManager()
										.beginTransaction()
										.replace(
