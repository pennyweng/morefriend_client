...
public class RaceAdapter extends ArrayAdapter<RaceItem> {
	...
	private RaceFragment fdf;


	public RaceAdapter(Context context, ArrayList<RaceItem> interestItems,
			DisplayImageOptions options, RaceFragment fdf ) {
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		final RaceItem postItem = getItem(position);
		View rowView = convertView;
//		if (rowView == null) {
			rowView = LayoutInflater.from(getContext()).inflate(
					R.layout.race_item, parent, false);
			TextView t = (TextView) rowView.findViewById(R.id.textView4);
			t.setText("作品#" + (position + 1));
